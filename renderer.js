(function () {
  ...
  let transferState = (() => { try { return JSON.parse(localStorage.getItem(TRANSFER_STATE_KEY) || '{}'); } catch (_) { return {}; } })();
  let settings      = (() => { try { return JSON.parse(localStorage.getItem(SETTINGS_KEY)       || '{}'); } catch (_) { return {}; } })();
  let scanStartTime = 0;
  ...
  function openBatchRenameModal(selected) {
    return new Promise((resolve) => {
      ...
      const onBackdropClick = (e) => { if (e.target === backdropEl) onCancel(); };
      const onKeydown = (e) => { if (e.key === 'Escape') onCancel(); };
  ...
  async function goClickHandler() {
    ...
    try {
      ...
      const proceedAfterConfirm = async () => {
        let conflicts = [];
        try {
          conflicts = await window.ppsaApi.checkConflicts(selected, dest, layout, customName);
        } catch (e) {
          err('checkConflicts error:', e);
          setResultModalBusy(false);
          setAppBusy(false);
          toast('Failed to check for conflicts: ' + (e.message || String(e)));
          return;
        }
  ...
  function toast(msg) {
    ...
    _toastTimer = setTimeout(() => { t.style.display = 'none'; _toastTimer = null; }, 3000);
  }
  ...
  function showNotification(title, body) {
    if (Notification.permission === 'granted') {
      ...
      }).catch(() => {}); // Suppress rejection in incognito / blocked contexts
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    try {
      ...
      // Display-only modals (no promise to resolve) — closed by a single global
      // Escape handler. Promise-based modals (confirm/conflict/rename/batch/subfolder)
      // register their own Escape handlers that resolve, so they are excluded here.
      const DISPLAY_ONLY_MODAL_IDS = [
        'gameDetailBackdrop', 'historyModalBackdrop', 'statsModalBackdrop',
        'verifyModalBackdrop', 'diffModalBackdrop', 'apiSettingsBackdrop',
        'ftpStorageModalBackdrop',
      ];
      ...
      document.addEventListener('keydown', e => {
        ...
        if (e.key === 'Escape') {
          // Close the topmost visible display-only modal, if any.
          for (let i = DISPLAY_ONLY_MODAL_IDS.length - 1; i >= 0; i--) {
            const m = $(DISPLAY_ONLY_MODAL_IDS[i]);
        ...
        if (e.ctrlKey && e.key === 'a') {
          if (typing) return; // allow native select-all in inputs
          e.preventDefault();
          if (viewMode === 'card') {
        ...
        } else if (!typing && (e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
          ...
          for (const ps5 of discovered) {
            ...
            chip.addEventListener('click', async () => {
              ...
              try {
                ...
                console.error('PS5 connect scan error:', e);
                toast('Scan failed: ' + e.message);
                activeChip = null;
                renderChips();
              } finally {
